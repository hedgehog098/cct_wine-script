...
// 清除缓存确认状态
let clearCacheConfirmTime = 0;                 // 0 表示未确认
const CLEAR_CACHE_CONFIRM_WINDOW = 10000;      // 10 秒
...
const STOP_KEY_RESOLVED = resolveKey(STOP_KEY);
const CLEAR_CACHE_KEY_RESOLVED = resolveKey(CLEAR_CACHE_KEY);
...
var keyEvent = JsMacros.on("Key", JavaWrapper.methodToJava((e) => {
    if (e.action != 1) return;                 // 只处理按下

    // ---- 强制退出 ----
    if (e.key == STOP_KEY_RESOLVED) {
        stopRequested = true;
    ...
    // ---- 清除木桶缓存（二次确认） ----
    if (e.key == CLEAR_CACHE_KEY_RESOLVED) {
        const now = Date.now();

        if (clearCacheConfirmTime > 0 && now - clearCacheConfirmTime <= CLEAR_CACHE_CONFIRM_WINDOW) {
            // 10 秒内第二次按下：确认删除
            clearCacheConfirmTime = 0;
            Chat.log("§c确认删除木桶缓存文件...");
            deleteBarrelCache();
        } else {
            // 第一次按下：记录时间并提示
            clearCacheConfirmTime = now;
            Chat.log("§e再次按下 " + CLEAR_CACHE_KEY + " 键将在 10 秒内删除木桶缓存文件。");
        }
    }
}));
...
function writeStringToFile(path, text) {
    ...
    try {
        writer.write(text);   // text 已经通过 JSON.stringify 得到，直接写
    } finally {
        writer.close();
    }
}
...
function readStringFromFile(path) {
    ...
    // 用 Java 的 String 构造器解码，避免 JS 侧累加
    const javaStr = new JavaString(bytes, StandardCharsets.UTF_8);
    // 转回 JS 字符串（这一步不会截断）
    return String(javaStr);
}
...
function loadBarrelTasksFromFile() {
    ...
    try {
        const text = readStringFromFile(BARREL_CACHE_FILE).trim();

        if (!text) {
            Chat.log("§e木桶缓存为空，删除后重新扫描。");
            deleteFileIfExists(BARREL_CACHE_FILE);
            return null;
        }

        const data = JSON.parse(text);

        if (!data || !Array.isArray(data.barrels)) {
            Chat.log("§e木桶缓存结构异常，删除后重新扫描。");
            deleteFileIfExists(BARREL_CACHE_FILE);
            return null;
        }
